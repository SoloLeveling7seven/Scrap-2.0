...
from bot import Bot, Ubot as ubot
...
async def scrap_(client: Client, message: Message):
    ...
    if not await present_user(id):
        try:
            ...
            pass
    start = (await client.ask(chat_id = id, text = "Send Start Message Link")).text
    End = (await client.ask(chat_id = id, text = "Send End Message Link")).text
    parts = start.split('/')
    channel_id = parts[-2].strip()
    channel_id = "-100"+channel_id
    channel_id = int(channel_id)
    message_id = parts[-1].strip()
    start = int(message_id)
    End = End.split('/')
    End = End[-1].strip()
    End = int(End)
    for i in range(start,End + 1):
        try:
            ...
            if old_post != None:
                await old_post.copy(int(-1002196752024))
...
@ubot.on_message((filters.video | filters.photo | filters.document) & filters.private)
        ...
        try:             
            ...
            if message.video:
                ...
                try:
                    ...
                    thumb = None
                post_message = await client.send_video(chat_id = int(-1002196752024), video = file,thumb = thumb)   
